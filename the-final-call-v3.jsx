...
// Canonical programme groups keep cross-college comparison accurate even when
// institutions use different display names for the same programme.
const BRANCH_GROUPS = {
  CSE: {
    label: "Computer Science and Engineering",
    aliases: {
      MSRIT: "COMPUTER SCIENCE AND ENGINEERING",
      RVCE: "COMPUTER SCIENCE AND ENGINEERING",
      PES: "B.TECH IN COMPUTER SCIENCE AND ENGINEERING",
      BMSCE: "COMPUTER SCIENCE AND ENGINEERING",
    },
  },
  CSE_AIML: {
    label: "Computer Science & Engineering — AI & ML",
    aliases: {
      MSRIT: "COMPUTER SCIENCE AND ENGG(ARTIFICIAL INTELLIGENCE AND MACHINE LEARNING)",
      RVCE: "COMPUTER SCIENCE AND ENGG(ARTIFICIAL INTELLIGENCE AND MACHINE LEARNING)",
      PES: "B.TECH IN COMPUTER SCIENCE & ENGINEERING (AI & ML)",
    },
  },
  CSE_DATA_SCIENCE: {
    label: "Computer Science & Engineering — Data Science",
    aliases: {
      RVCE: "COMPUTER SCIENCE AND ENGINEERING(DATA SCIENCE)",
      BMSCE: "COMPUTER SCIENCE AND ENGINEERING (DATA SCIENCE)",
    },
  },
  CSE_CYBER: {
    label: "Computer Science & Engineering — Cyber Security",
    aliases: {
      MSRIT: "COMPUTER SCIENCE AND ENGINEERING (CYBER SECURITY)",
      RVCE: "COMPUTER SCIENCE AND ENGINEERING (CYBER SECURITY)",
    },
  },
  AI_ML: {
    label: "Artificial Intelligence and Machine Learning",
    aliases: {
      MSRIT: "ARTIFICIAL INTELLIGENCE AND MACHINE LEARNING",
      BMSCE: "ARTIFICIAL INTELLIGENCE AND MACHINE LEARNING",
    },
  },
  AI_DS: {
    label: "Artificial Intelligence and Data Science",
    aliases: {
      MSRIT: "ARTIFICIAL INTELLIGENCE AND DATA SCIENCE",
      BMSCE: "ARTIFICIAL INTELLIGENCE AND DATA SCIENCE",
    },
  },
  ECE: {
    label: "Electronics and Communication Engineering",
    aliases: {
      MSRIT: "ELECTRONICS AND COMMUNICATION ENGG",
      RVCE: "ELECTRONICS AND COMMUNICATION ENGG",
      PES: "B.TECH IN ELECTRONICS & COMMUNICATION ENGINEERING",
      BMSCE: "ELECTRONICS AND COMMUNICATION ENGG",
    },
  },
  EEE: {
    label: "Electrical & Electronics Engineering",
    aliases: {
      MSRIT: "ELECTRICAL & ELECTRONICS ENGINEERING",
      RVCE: "ELECTRICAL & ELECTRONICS ENGINEERING",
      PES: "B.TECH IN ELECTRICAL & ELECTRONICS ENGINEERING",
    },
  },
  MECH: {
    label: "Mechanical Engineering",
    aliases: {
      MSRIT: "MECHANICAL ENGINEERING",
      RVCE: "MECHANICAL ENGINEERING",
      PES: "B.TECH IN MECHANICAL ENGINEERING",
      BMSCE: "MECHANICAL ENGINEERING",
    },
  },
  BIOTECH: {
    label: "Bio-Technology",
    aliases: {
      MSRIT: "BIO-TECHNOLOGY",
      RVCE: "BIO-TECHNOLOGY",
      PES: "B.TECH IN BIO-TECHNOLOGY",
      BMSCE: "BIO-TECHNOLOGY",
    },
  },
  CHEMICAL: {
    label: "Chemical Engineering",
    aliases: {
      MSRIT: "CHEMICAL ENGINEERING",
      RVCE: "CHEMICAL ENGINEERING",
      BMSCE: "CHEMICAL ENGINEERING",
    },
  },
  CIVIL: {
    label: "Civil Engineering",
    aliases: {
      MSRIT: "CIVIL ENGINEERING",
      RVCE: "CIVIL ENGINEERING",
      BMSCE: "CIVIL ENGINEERING",
    },
  },
  AEROSPACE: {
    label: "Aerospace Engineering",
    aliases: {
      MSRIT: "AERO SPACE ENGINEERING",
      RVCE: "AERO SPACE ENGINEERING",
    },
  },
  ETE: {
    label: "Electronics and Telecommunication Engineering",
    aliases: {
      MSRIT: "ELECTRONICS AND TELECOMMUNICATION ENGINEERING",
      RVCE: "ELECTRONICS AND TELECOMMUNICATION ENGINEERING",
    },
  },
  IEM: {
    label: "Industrial Engineering & Management",
    aliases: {
      MSRIT: "INDUSTRIAL ENGINEERING & MANAGEMENT",
      RVCE: "INDUSTRIAL ENGINEERING & MANAGEMENT",
    },
  },
};

function branchGroupFor(branch) {
  if (!branch) return null;
  return Object.entries(BRANCH_GROUPS).find(([, group]) =>
    Object.values(group.aliases).includes(branch)
  ) || null;
}

function resolveEquivalentBranch(college, sourceBranch) {
  if (!college || !sourceBranch) return null;

  const groupEntry = branchGroupFor(sourceBranch);
  if (groupEntry) {
    const [, group] = groupEntry;
    const mapped = group.aliases[college];
    if (mapped && RAW_DATA[college]?.[mapped]) return mapped;
    return null;
  }

  // Safe fallback for programmes whose names are already identical.
  return RAW_DATA[college]?.[sourceBranch] ? sourceBranch : null;
}

function canonicalBranchLabel(branch) {
  const groupEntry = branchGroupFor(branch);
  return groupEntry ? groupEntry[1].label : branch;
}
...
function MiniCard({ college, branch, category, rank }) {
  const resolvedBranch = resolveEquivalentBranch(college, branch);
  const cutoff = resolvedBranch ? RAW_DATA[college]?.[resolvedBranch]?.[category] : undefined;
  const hasCutoff = cutoff !== null && cutoff !== undefined;
  const v = hasCutoff ? verdictFor(((cutoff - rank) / cutoff) * 100) : null;
  const s = !hasCutoff
    ? { bg: "#f4f1ea", accent: "#9a9284", text: "#7a745f", border: "#d8d2c2" }
    : VERDICT_STYLE[v.key];
  const cls = !hasCutoff ? "" : VERDICT_CLASS[v.key];

  return (
    <div className={cls} style={{
      border: `1px solid ${s.border}55`, borderTop: `4px solid ${s.accent}`,
      background: s.bg, borderRadius: 4, padding: "16px 16px", flex: "1 1 200px", minWidth: 180,
    }}>
      <div style={{ fontFamily: "'Source Serif Pro', Georgia, serif", fontWeight: 700, fontSize: 14, color: "#2a2620", marginBottom: 5 }}>
        {college}
      </div>
      <div style={{
        fontFamily: "'IBM Plex Mono', monospace", fontSize: 9, lineHeight: 1.35,
        color: "#8a8472", minHeight: 25, marginBottom: 7,
      }}>
        {resolvedBranch || "Equivalent programme not available"}
      </div>
      {!resolvedBranch ? (
        <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, color: "#7a745f" }}>
          Not offered / not mapped in this dataset
        </div>
      ) : !hasCutoff ? (
        <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, color: "#7a745f" }}>
          No {category} cutoff data
        </div>
      ) : (
        <React.Fragment>
          <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, color: "#8a8472", marginBottom: 2 }}>
            Cutoff: <AnimatedNumber value={cutoff} duration={700} />
          </div>
          <div className="tfc-badge-anim" style={{
...
export default function TheFinalCall() {
  ...
  const suggestions = useMemo(() => {
    if (!submitted || submitted.compareMode) return [];
  ...
  const resultKey = submitted
    ? `${submitted.college}|${submitted.branch}|${submitted.category}|${submitted.rank}|${submitted.compareMode}|${submitted.runId}`
    : "empty";

  const resultTheme = useMemo(() => {
    if (!submitted) return "neutral";

    const resolvedBranch = resolveEquivalentBranch(submitted.college, submitted.branch) || submitted.branch;
    const cutoff = RAW_DATA[submitted.college]?.[resolvedBranch]?.[submitted.category];
  ...
  return (
    ...
    >
      ...
      <div className="tfc-page-inner" style={{ maxWidth: 880, margin: "0 auto" }}>
        ...
        <form onSubmit={handleSubmit} style={{
          ...
          <div style={{
            ...
            <label style={{
              ...
              <input type="checkbox" checked={compareMode} onChange={handleCompareChange} />
              Compare equivalent branch + category across all 4 colleges
            </label>
        ...
        {!submitted ? (
          ...
          <div key={resultKey}>
            <div style={{
              ...
              color: "#7a745f", textTransform: "uppercase", marginBottom: 10,
            }}>
              Comparing equivalent programme: {canonicalBranchLabel(submitted.branch)}
            </div>
        ...
        {submitted && !submitted.compareMode && suggestions.length > 0 && (
